...
{
   // set the capture
   capture = board(dest.getRow(), dest.getCol())->getLetter();

   // set the color
   isWhite = board(source.getRow(), source.getCol())->isWhite();

   // handle if this is an en-passant
   if (capture == SPACE && board(source.getRow(), source.getCol())->getLetter() == PAWN)
      enpassant = true;
}
